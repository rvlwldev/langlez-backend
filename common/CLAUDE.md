...
**역할**: common — 웹·보안·예외·필터·i18n 공용
**경로**: `common`
**의존 모듈**: `core`, `module:member-api`

---

## 0. 경계 — 이 디렉터리 밖으로 나가지 않는다

**이 모듈 디렉터리(`common`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
웹·보안·예외·필터·i18n 공용 코드. 모든 도메인 모듈이 이 모듈을 의존한다. **여기서 깨지면 전 모듈이 깨진다.**

- `JwtAuthenticationFilter` — 매 요청 토큰 검증 + 계정 상태 확인. 이 때문에 `common` 은 `module/member-api` 계약을 의존한다(계약 모듈은 의존성 0 이라 순환은 없다).
- `LanglezException` + `GlobalRestControllerAdvice` — 예외를 상태코드 + i18n 키로 변환한다.
- `@MemberId` 아규먼트 리졸버 — 인증된 회원 id 주입. **회원 id 를 요청 본문으로 받지 않는 근거가 이것이다.**
- `WebSocketSubscriptionGate` — 모든 STOMP SUBSCRIBE 의 단일 인가 지점.
- `PerformanceLogger` — 쿼리 로깅 단일 창구. 임계값은 `logger.{rdb,mongo,redis}.{log-threshold-ms, warn-threshold-ms}`, `warn-threshold` 이상만 `WARN`.
- `messages_*.properties` 12개 + `logback-spring.xml`.

## 8. 절대 건드리면 안 되는 것

**`src/main/resources/messages.properties`(로케일 접미사 없는 파일)를 지우지 않는다.** 비어 있어도 있어야 한다. Spring Boot 의 `MessageSourceAutoConfiguration` 은 `classpath*:{basename}.properties` 가 실제로 있는지만 보고 `messageSource` 빈 생성 여부를 정한다. 이 파일이 없으면 12개 번들이 다 있어도 빈이 안 생기고 `DelegatingMessageSource` 가 그 자리를 채워 **모든 키 조회가 실패한다** — 키 하나가 아니라 전 응답이 키 문자열로 나간다. 실제로 그 상태였다. 회귀 방지는 `app/api` 의 `MessageSourceAutoConfigurationTest` 가 한다.

**`logback-spring.xml` 을 "중복"으로 보고 지우지 않는다.** `app/api` 와 여기 양쪽에 의도적으로 둔다. `module/*`·`infra/*` 는 `app/api` 를 의존하지 않아 단독 테스트에서는 `app/api` 의 logback 이 클래스패스에 없다. 여기 것을 지우면 그 실행 경로가 아무 설정 없이 돈다.

**i18n 키는 12개 번들 전부에 넣는다.** (ko, ja, en, de, es, fr, pt, id, ru, vi, zh_CN, zh_TW) 누락되면 키 문자열이 그대로 사용자 응답에 노출된다.

```bash
for f in src/main/resources/messages_*.properties; do echo "$(basename $f) $(grep -c '^[a-z].*=' $f)"; done
```
...
`WebSocketSubscriptionGate` 는 등록된 `core.SubscriptionAuthorizer` 중 `supports` 가 참인 것에게 묻고 **하나도 없으면 거부한다.** 기본 거부다.

모듈마다 `ChannelInterceptor` 를 달고 "내 접두사가 아니면 통과"시키던 예전 구조는 실제로 뚫렸다 — 어느 접두사에도 안 걸리는 목적지를 아무도 검사하지 않아 별표 두 개짜리 구독 패턴과 `/topic/notification/{id}` 가 열려 있었다. **이 기본 거부를 되돌리지 않는다.** 새 토픽은 도메인 모듈이 authorizer 를 추가하는 것으로 끝내야 한다.

`fail-open` 검증(`x != null && x != y`)을 만들지 않는다. 헤더를 빼기만 하면 검증이 통째로 우회된다. 1인 1기기 정책의 `X-Device-Id` 검사가 특히 그렇다.
