...
**이 모듈 디렉터리(`core`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
**소유자가 인프라인 순수 계약만** 담는다. `CacheProvider`/`Cache`, `MessageBroadcaster`, `MessageDeduplicator`, `SubscriptionAuthorizer`.

- **의존성 0.** Spring·JPA 를 넣지 않는다. JDK 타입만 쓴다. 어느 모듈이든 순환 없이 물 수 있어야 한다.
- **구현은 여기 없다.** `infra/redis` 등이 구현한다.
- **여기 둘지 `{도메인}-api` 에 둘지의 기준은 소유자다.** 그 계약을 구현하고 데이터를 소유하는 게 도메인 모듈이면 `{도메인}-api`, 인프라면 `core`. **애매하면 `core`** — 소유자가 하나로 정해지지 않는 계약을 억지로 도메인에 밀어 넣으면 그 도메인을 안 쓰는 모듈까지 끌려온다.
- `SubscriptionAuthorizer` 는 방향이 뒤집힌 예다. `common` 이 소비하고 도메인들이 구현한다 — 어느 도메인의 것도 아니라서 여기 있다.

## 8. 시그니처를 바꿀 때

**이 모듈의 인터페이스는 사실상 전 모듈이 본다.** 바꾸면 광범위하게 깨진다. 다른 모듈 파일을 직접 열지 말고, 무엇을 어떻게 바꾸려는지와 영향 범위를 **사용자에게 보고한다.**

- `CacheProvider` 에는 **multi-get/multi-set 이 있어야 한다.** Spring `Cache` 에 그게 없어서 컬렉션 조회가 건당 왕복으로 쪼개진 것이 이 포트를 따로 만든 이유 중 하나다.
- `MessageBroadcaster` 는 인메모리 STOMP 브로커의 인스턴스 격리를 메우는 포트다. 서비스 코드가 `SimpMessagingTemplate` 을 직접 쓰지 않게 하는 것이 목적이다.
- 포트 메서드에는 KDoc 으로 **감수하기로 한 정합성 창**을 적는다.
