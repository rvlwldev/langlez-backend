...
**역할**: 인프라 모듈 `rdb`
**경로**: `infra/rdb`
**의존 모듈**: `common`, `core`, `module:member-api`

---

## 0. 경계 — 이 디렉터리 밖으로 나가지 않는다

**이 모듈 디렉터리(`infra/rdb`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
```bash
./gradlew :infra:rdb:test          # 이 모듈 테스트
...
`infra/*` 는 도메인이 모르는 기술 세부를 감춘다. **도메인 모듈을 의존하지 않는다** (예외: `infra/rdb` 와 `common` 은 계정 상태 조회 때문에 `module/member-api` 계약만 의존한다 — 계약 모듈은 의존성 0 이라 순환이 생기지 않는다).

- 이 모듈이 `api()` 로 노출하는 것(JPA, web, validation, swagger 등)을 소비 모듈이 다시 선언하지 않게 한다. 노출 범위를 바꾸면 소비 모듈 전부에 영향이 간다.
- **`application.yml` 을 만들지 않는다.** 기본값이 필요하면 `@ConfigurationProperties`/`@Value` 로 코드에 둔다.
- 포트 인터페이스는 이 모듈이 아니라 **`core`** 에 둔다(`CacheProvider`, `MessageBroadcaster`, `MessageDeduplicator`). 여기엔 구현만 있다.
- 자동설정 빈은 조건을 명확히 건다. 조용히 안 생기는 빈이 가장 찾기 어렵다.

## 8. 이 모듈이 소유한 것

JPA + QueryDSL 설정, Outbox 베이스(`OutBox`, `OutBoxRepository`, `OutBoxProcessor`, `OutBoxHistoryCleaner`), P6Spy 쿼리 로깅, Flyway 마이그레이션(`src/main/resources/migration/V{n}__*.sql`), `StringPath.search()`(pg_trgm + unaccent) 확장.

**Flyway**

- 운영·개발·테스트 **모두 `ddl-auto: validate`**. 통합테스트도 Flyway 를 타므로 마이그레이션 자체가 검증된다.
- **이미 적용된 V 파일은 절대 수정하지 않는다.** 체크섬 불일치로 기동이 실패한다. 새 V 파일을 만든다.
- **인덱스**: 운영에 행이 없으면 일반 `create index`. **`concurrently` 를 기본으로 삼지 마라** — 기존의 모든 트랜잭션이 끝나기를 기다리고, 커넥션 풀이 스냅샷을 잡고 있으면 영영 안 끝난다. 실제로 `V8` 을 `concurrently` 로 썼을 때 통합테스트가 **3시간 반** 멈췄고, 일반 `create index` 로 바꾸니 전체 빌드가 2분 52초에 끝났다. 행이 쌓인 뒤라면 데이터를 줄이는 마이그레이션을 먼저 배포하고 `concurrently` 를 별도 V 파일로 분리한다(그 스크립트엔 `-- flyway executeInTransaction=false` 가 필요하고, 실패하면 `INVALID` 인덱스가 남는다).
- 어느 쪽을 택하든 **이유를 SQL 주석에 남긴다.**
- `@Column(nullable = false)` 가 새로 붙으면 **기존 행 백필 마이그레이션이 반드시 따라온다.**
- **`StringPath.search()` 를 적용한 컬럼에는 `f_unaccent()` GIN 인덱스를 반드시 건다.** 안 걸면 쿼리는 통과하지만 매번 Seq Scan 이다. 컬럼과 검색어 양쪽이 같은 `f_unaccent()` 를 거쳐야 인덱스를 탄다.

**Outbox 베이스**

- `OutBoxProcessor` 의 튜닝 상수는 `open val`(`chunk`, `tries`, `threads`). **`open val` 을 베이스 생성자에서 읽지 않는다** — 하위 클래스 값이 아직 0 이라 `Semaphore(0)` 으로 전체가 멈춘다. `by lazy` 로 미룬다.
- **프록시 대상 클래스의 메서드는 `open` 이어야 한다.** `OutBoxHistoryCleaner.cleanBefore` 가 `final` 이라 CGLIB 프록시가 오버라이드를 못 해 필드가 빈 프록시에서 실행됐고 `lateinit ... has not been initialized` 로 터졌다.

**쿼리 로깅** — `common` 의 `PerformanceLogger` 한 곳을 통해 나간다. 임계값은 `logger.rdb.{log-threshold-ms, warn-threshold-ms}`, **`warn-threshold` 이상만 `WARN` 이고 나머지는 `DEBUG`** — INFO 로 올리면 콘솔이 스케줄러 로그로 덮인다.

**QueryDSL** — Q타입은 별칭 import. 카운터 갱신은 `dsl.update(...)` 단일 UPDATE 로 원자화하고 감소에는 0 미만 방지 조건을 건다.
