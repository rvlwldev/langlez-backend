...
**이 모듈 디렉터리(`infra/kafka`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
```bash
./gradlew :infra:kafka:test          # 이 모듈 테스트
...
프로듀서·컨슈머 설정, DLT(Dead Letter Topic) 처리.

- **Kafka 는 "유실되면 안 되는 모듈 간 상태 변경 전파" 에만 쓴다.** 이벤트 발행은 아웃박스를 경유한다(발행 모듈의 `infrastructure/outbox`).
- **고빈도·저가치 신호를 브로커에 태우지 않는다.** 접속 핑 같은 하트비트는 Redis 직결이다.
- 이벤트 DTO 는 이 모듈이 아니라 **발행 도메인의 `module/{domain}-api`** 에 있다. 여기엔 전송 설정만 둔다.
- 컨슈머 구현은 소비 도메인 모듈의 `api/{Domain}Consumer.kt` 에 둔다. **외부 메시지 계약과 내부 모델이 다르면 컨슈머에서 변환하고 이유를 주석으로 남긴다.**
- 역직렬화 실패·처리 실패는 DLT 로 보낸다. 조용히 삼키면 유실이 된다.
