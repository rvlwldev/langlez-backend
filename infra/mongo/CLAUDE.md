...
**이 모듈 디렉터리(`infra/mongo`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
```bash
./gradlew :infra:mongo:test          # 이 모듈 테스트
...
Mongo 리포지토리 스캔 설정과 인덱스 초기화.

- **MongoDB 에 담는 것은 채팅 메시지 본문 + 첨부다.** 무한 증가하고, 첨부를 임베드해 조회 1회로 끝내려는 선택이다. 조인·트랜잭션이 필요한 데이터(회원·프로필·방·참여자·아웃박스)는 PostgreSQL 에 있다.
- **인덱스는 기동 시 초기화 코드로 보장한다.** 문서 저장소는 스키마 검증이 없어 인덱스 누락이 조용히 넘어가고 컬렉션이 커진 뒤에야 드러난다.
- **가장 빈번한 쓰기(채팅 메시지)는 별도 아웃박스 행 대신 문서의 `published` 플래그**로 발행 상태를 관리한다. 쓰기 증폭을 없애려는 의도된 예외다.
- 정렬·커서는 시각이 아니라 **시퀀스** 기준으로 잡는다. 인스턴스 간 시계 차이로 순서가 뒤집힌다.
