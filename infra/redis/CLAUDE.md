...
**이 모듈 디렉터리(`infra/redis`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
```bash
./gradlew :infra:redis:test          # 이 모듈 테스트
...
Redisson 설정, `core.CacheProvider` 구현(캐시 어댑터), `@DistributedLock` AOP, Redis pub/sub 기반 `MessageBroadcaster`, `MessageDeduplicator`, 접속/화면 상태 버킷, wave 채팅 저장, `DailyRateLimiter`.

**코덱 함정 — 이 모듈에서 가장 많이 터진 것**

- **Redisson 코덱이 final 타입에 `@class` 를 안 붙이면 캐시 read 가 100% 실패한다.** 무한 플래핑으로 이어진다.
- **`Long` 을 Redis 셋에 저장하면 JSON 코덱이 `Integer` 로 돌려줘 `contains(1L)` 이 조용히 false 가 된다.** 타입을 고정하거나 문자열로 저장한다.
- **Lua(EVAL)를 직접 짜기 전에 `@DistributedLock` 을 먼저 고려한다.** 기본 코덱은 바이너리 직렬화라 Lua 인자가 원시 바이트로 넘어가 `tonumber()`/`SISMEMBER` 비교가 깨진다. 꼭 필요하면 `getScript(StringCodec.INSTANCE)` 로 코덱을 명시한다 — `DailyRateLimiter` 가 그 방식이다.

**`@DistributedLock`**

옵션: `prefix`, `keys`(SpEL 배열), `leaseSecs`(0 이하면 자동 갱신), `waitMs`·`retries`, `transactional`(락 획득 후 트랜잭션 시작), `throwOnFailure`(기본 `false` — 실패 시 조용히 스킵). 락 키는 파라미터에 `@LockKey` 를 붙이거나 `keys` 에 SpEL 을 준다.

- **self-invocation 에서 안 돈다.** 프록시 기반이라 같은 클래스 안 호출은 advice 를 타지 않는다. 별도 `@Component` 로 분리한다.
- **프록시 대상 메서드는 `open`.** `final` 이면 필드가 빈 프록시에서 그대로 실행된다.
- 스케줄러 중복 방지처럼 놓쳐도 다음 주기에 만회되면 `throwOnFailure = false`.

**고빈도 하트비트는 Redis 직결.** 접속 핑(5초 간격)을 Kafka 에 태웠다가 브로커 왕복 비용만 들었다. 지금은 레디스 버킷에 바로 쓴다.
