 ...
 */
class RelationshipOutBoxSchedulerTest : BehaviorSpec({

    Given("아웃박스 발행 스케줄러는") {

        val send = RelationshipOutBoxScheduler::class.java.getDeclaredMethod("send")
        ...
        When("send() 를 보면") {
            ...
            Then("중복 발행을 막는 @DistributedLock 이 함께 붙어 있다") {
                send.getAnnotation(DistributedLock::class.java)
                    .shouldNotBeNull().prefix shouldBe "lock:relationship-outbox"
            }
        }
    }
    ...
    Given("아웃박스 이력 이관 스케줄러는") {
        ...
        val archive = RelationshipOutBoxHistoryScheduler::class.java.getDeclaredMethod("archive")
        ...
        When("archive() 를 보면") {
            ...
            Then("중복 실행을 막는 @DistributedLock 이 함께 붙어 있다") {
                archive.getAnnotation(DistributedLock::class.java)
                    .shouldNotBeNull().prefix shouldBe "lock:relationship-outbox-history"
            }
        }
    }
})
