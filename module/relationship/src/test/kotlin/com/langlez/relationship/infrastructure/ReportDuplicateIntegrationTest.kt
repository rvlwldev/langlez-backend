...
import com.langlez.relationship.api.RelationshipConsumer
 ...
 *
 * 겹이 셋이다 — `MessageDeduplicator`(레디스), `existsReport`(앱), `UNQ_REPORT_IDENTITY`(DB).
 * 앞의 둘이 뚫리는 경로가 실재하므로(레디스 장애 fail-open, TTL 만료, 페이로드가 다른 재전달,
 * 그리고 check-then-insert 경합) 마지막 겹의 판정 범위를 여기서 못 박는다.
 *
 * 제약 자체는 JDBC 로 직접 찌른다. 서비스를 거치면 `existsReport` 가 먼저 걸러서
...
class ReportDuplicateIntegrationTest : BehaviorSpec() {
    ...
    @Autowired
    lateinit var consumer: RelationshipConsumer
