...
**역할**: 도메인 모듈 `member` — 4계층(api / application / domain / infrastructure)
**경로**: `module/member`
**의존 모듈**: `common`, `core`, `infra:kafka`, `infra:rdb`, `infra:redis`, `module:attachment-api`, `module:member-api`

> **이 모듈이 저장소 전체의 기준 모듈(reference module)이다.** 다른 모듈은 여기 구조와 관례를 그대로 따른다.
> 규약 문서와 이 모듈의 코드가 어긋나면 **코드가 정답이다.**

---

## 0. 경계 — 이 디렉터리 밖으로 나가지 않는다

**이 모듈 디렉터리(`module/member`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
```bash
./gradlew :module:member:test          # 이 모듈 테스트
...
**DTO** — `data class`, 모든 프로퍼티 `val`. **어노테이션에 `@field:` 타깃을 명시한다**(생략하면 파라미터에 붙어 런타임에 무시될 수 있다). 엔티티 → 응답 변환은 **보조 생성자**로 한다 (별도 Mapper 나 `toResponse()` 확장함수를 만들지 않는다). 응답은 노출 범위별로 나눈다 — 본인용 `MemberMeResponse`, 타인용 `MemberPublicResponse`. 하나의 DTO 에 nullable 을 섞어 재사용하지 않는다.
