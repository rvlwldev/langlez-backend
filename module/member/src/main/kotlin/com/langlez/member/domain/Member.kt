...
class Member(
    ...
    /** 사용자 지정 공개 식별자 (트위터 @username 같은 역할) */
    @Column(length = 20) var handle: String? = null,
    ...
    companion object {
        private val HANDLE_PATTERN = Regex("^[a-z0-9_]{3,20}$")
        ...
        fun create(
            ...
            Member(
                ...
                provider =
                    MemberProvider(
                        providerId,
                        MemberProvider.Type.valueOf(providerType),
                        providerUserName
                    ),
                audit = MemberAudit()
            )
