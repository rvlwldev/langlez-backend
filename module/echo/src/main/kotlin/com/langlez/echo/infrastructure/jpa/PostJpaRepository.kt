...
interface PostJpaRepository : JpaRepository<Post, Long> {
    @Query("SELECT p FROM Post p WHERE p.authorId IN :authorIds AND p.blinded = false AND p.deletedAt IS NULL AND (:cursor IS NULL OR p.id < :cursor) ORDER BY p.id DESC")
    fun findFollowingFeed(authorIds: List<Long>, cursor: Long?, pageable: PageRequest): List<Post>

    @Query("SELECT p FROM Post p WHERE p.authorId NOT IN :excludeAuthorIds AND p.blinded = false AND p.deletedAt IS NULL AND (:cursor IS NULL OR p.likeCount < :cursorLikeCount OR (p.likeCount = :cursorLikeCount AND p.id < :cursor)) ORDER BY p.likeCount DESC, p.id DESC")
    ...
    @Query("SELECT p FROM Post p WHERE p.blinded = false AND p.deletedAt IS NULL AND (:cursor IS NULL OR p.likeCount < :cursorLikeCount OR (p.likeCount = :cursorLikeCount AND p.id < :cursor)) ORDER BY p.likeCount DESC, p.id DESC")
