 ...
 */
@Component
internal class BlockOutBoxHistoryScheduler(repo: BlockOutBoxRepository) :
    OutBoxHistoryProcessor<BlockOutBox, BlockOutBoxHistory>(repo) {

    override val chunk = 1000

    override fun toHistory(outbox: BlockOutBox) = BlockOutBoxHistory(outbox)

    @Scheduled(cron = "0 0 6 * * *") // 매일 아침 6시
    @DistributedLock(prefix = "lock:block-outbox-history")
