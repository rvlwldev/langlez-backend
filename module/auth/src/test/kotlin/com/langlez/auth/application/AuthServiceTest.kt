...
class AuthServiceTest :
        BehaviorSpec({
            ...
            Given("유효한 refresh token으로 갱신 요청 시") {
                ...
                every { tokenProvider.createAccessToken(email, "ROLE_MEMBER") } returns
                        "new_access_token"
            ...
            Given("잘못된 JWT 형식의 refresh token 입력 시") {
                ...
                every { tokenProvider.getEmail(invalidToken) } throws
                        RuntimeException("Invalid JWT")
            ...
            Given("Token Rotation 검증") {
                ...
                every { tokenProvider.validateToken(refreshToken) } returns true
                every { memberService.getMember(email) } returns member
                every { tokenProvider.createAccessToken(email, "ROLE_PREMIUM") } returns
                        "new_access"
                every { tokenProvider.createRefreshToken(email) } returns "new_refresh"
                every { redisTemplate.delete("refresh_token:$email") } returns true
                every {
                    valueOperations.set("refresh_token:$email", "new_refresh", 14, TimeUnit.DAYS)
                } returns Unit
