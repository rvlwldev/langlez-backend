...
class FollowControllerTest : BehaviorSpec({

    val service = mockk<FollowService>()
    val controller = FollowController(service)

    afterEach { clearMocks(service, answers = false) }

    fun view(memberId: Long) = FollowMemberView(memberId, memberId, "user$memberId", null)
