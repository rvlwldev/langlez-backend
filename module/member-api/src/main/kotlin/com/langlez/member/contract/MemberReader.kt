...
interface MemberReader {
    ...
    data class ProfileInfo(
        ...
        /**
         * **기본값을 두지 않는다.** 목록에서 정지·탈퇴 회원을 걸러내는 유일한 근거라,
         * 기본값이 있으면 구성 지점 하나가 빠졌을 때 그 회원이 정상으로 보인다.
         *
         * 이 필드가 없던 시절에는 [findStatus] 를 후보마다 단건으로 부르는 수밖에 없었고,
         * 이 포트가 gRPC 로 나가면 목록 길이만큼 왕복이 됐다. 배치 조회 하나로 끝내려고 여기 담는다.
         */
        val status: Status,
    )
