 ...
 * 반대로 `@DistributedLock` 만 붙으면 아무도 정리하지 않아 `chat_event_outbox_history` 가 무한 증가한다.
 */
class ChatOutBoxHistoryCleanupSchedulerTest : BehaviorSpec({

    Given("채팅 아웃박스 이력 정리 스케줄러는") {

        val clean = ChatOutBoxHistoryCleanupScheduler::class.java.getDeclaredMethod("clean")
        ...
        When("clean() 을 보면") {
            ...
            Then("중복 실행을 막는 @DistributedLock 이 함께 붙어 있다") {
                clean.getAnnotation(DistributedLock::class.java)
                    .shouldNotBeNull().prefix shouldBe "lock:chat-outbox-history-cleanup"
            }
        }
    }
})
