# module/matching — 모듈 규약

**역할**: 도메인 모듈 `matching` — 4계층(api / application / domain / infrastructure)
**경로**: `module/matching`
**의존 모듈**: `common`, `core`, `module:block-api`, `module:follow-api`, `module:lang-api`, `module:matching-api`, `module:member-api`

> **이 모듈은 4계층의 유일한 예외다.** `api`/`application` 만 갖고 `domain`·`infrastructure` 가 없다.
...
> **다른 모듈에 이 예외를 복사하지 마라** — 판단 기준은 "자기 소유 데이터가 있는가"다.

---

## 0. 경계 — 이 디렉터리 밖으로 나가지 않는다

**이 모듈 디렉터리(`module/matching`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
```bash
./gradlew :module:matching:test          # 이 모듈 테스트
...
```
module/matching/src/main/kotlin/com/langlez/matching/
├── api/                        # 외부 진입점 (HTTP, Kafka, 애플리케이션 이벤트)
│   ├── MatchingAPI.kt               # Swagger 문서 전용 인터페이스
│   ├── MatchingController.kt        # MatchingAPI 구현, Spring MVC 매핑만
│   ├── MatchingEventListener.kt     # @TransactionalEventListener → Outbox 기록
│   ├── request/                # 요청 DTO
│   └── response/               # 응답 DTO
├── application/                # 유스케이스 조합, 트랜잭션 경계
├── domain/                     # 엔티티 + 저장소 포트(인터페이스)
└── infrastructure/             # 포트의 구현(어댑터)
    ├── MatchingRepositoryImpl.kt
...
**DTO** — `data class`, 모든 프로퍼티 `val`. **어노테이션에 `@field:` 타깃을 명시한다**(생략하면 파라미터에 붙어 런타임에 무시될 수 있다). 엔티티 → 응답 변환은 **보조 생성자**로 한다 (별도 Mapper 나 `toResponse()` 확장함수를 만들지 않는다). 응답은 노출 범위별로 나눈다 — 본인용 `MatchingMeResponse`, 타인용 `MatchingPublicResponse`. 하나의 DTO 에 nullable 을 섞어 재사용하지 않는다.
