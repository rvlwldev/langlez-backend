...
Orca orchestration 스킬을 쓰거나 그 밖에 서브에이전트를 띄울 때 모델을 골라 넘긴다. **구현은 sonnet 이 기본**이고, 정말 가벼운 기계적 작업만 **haiku** 로 내린다.

| 모델 | 맡기는 일 |
|---|---|
| **haiku** | 판단이 필요 없는 기계적 작업. i18n 12개 번들에 같은 키 채워넣기, 파일 위치·심볼 찾기, 단순 문자열 치환, 목록 수집 |
| **sonnet** | **구현·수정 전부.** DTO·컨트롤러 매핑, Flyway V 파일, 모듈 신설, 동시성·정합성 수정, 모듈 경계 작업까지 |
| **opus** | 이 문서 자체의 갱신, 그리고 사용자가 명시적으로 요청할 때 |

**구현은 sonnet 으로 띄운다.** 예전에는 "조용히 잘못될 수 있는 것"을 opus 로 올렸는데, 그 방어선은 모델 등급이 아니라 **리뷰와 테스트**가 맡는다. 실제로 이 저장소에서 조용히 잘못된 것들 — 부분 인덱스가 바인드 파라미터라 안 걸리는데 테스트는 초록불, `jti` 부재로 같은 초 발급 토큰이 회전되지 않음, 세션 등록과 바인딩 사이 창으로 맵이 누수 — 은 전부 **리뷰가 잡았지 모델 등급이 잡은 게 아니다.**

대신 **지시서를 더 촘촘히 쓴다.** sonnet 이 흘리기 쉬운 것들을 매번 명시한다 — `@field:` 타깃 누락, 엔티티를 `data class` 로 선언, 조건을 메서드명으로 이은 파생 쿼리, `@Scheduled` 에 `@DistributedLock` 누락, `!!` 사용.
...
1. **수정 전 코드에서 새 테스트가 빨간불인 것을 먼저 확인한다.**
   확인하고 그 실패 메시지를 보고에 적는다. 빨간불이 안 되면 그 테스트는 결함을 안 덮는 것이다.
   이 절차가 실제로 여러 번 값을 했다 — `#45` 의 인덱스 테스트는 리터럴 SQL 로 EXPLAIN 해서
   프로덕션 경로(바인드 파라미터)를 안 덮은 채 초록불이었다. 그대로 머지했으면 **아무것도 안 고친 PR** 이 됐다.
2. **테스트가 프로덕션 경로를 타는지 본다.** 리터럴 SQL, 손으로 짠 대역, 내부 메서드 직접 호출은
   실제 경로를 안 덮을 수 있다. Testcontainers 로 진짜 인프라를 태우는 쪽이 확실하다.
3. **동시성 결함은 실제 동시성으로 검증한다.** mockk 로는 못 잡는다.
   `ReportConcurrencyIntegrationTest`, `MemberStatusCacheRaceTest` 가 선례다.
4. **스키마·설정 변경은 런타임으로 확인한다.** 마이그레이션 파일을 만든 것으로는 부족하다.
   `bootRun` 으로 적용하고 `\d`·`EXPLAIN`·실제 빈 속성을 눈으로 본다.
   측정용 데이터를 넣었으면 **반드시 원복한다.**
5. **어노테이션 누락은 리플렉션으로 고정한다.** `@Scheduled`·`@DistributedLock` 이 빠져도
   컴파일과 테스트가 통과한다. 아웃박스 스케줄러 테스트들이 선례다.
