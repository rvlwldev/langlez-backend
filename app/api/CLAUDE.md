...
**역할**: app/api — 조립과 실행
**경로**: `app/api`
**의존 모듈**: `common`, `infra:kafka`, `infra:mongo`, `infra:rdb`, `infra:redis`, `module:attachment`, `module:auth`, `module:block`, `module:chat`, `module:echo`, `module:follow`, `module:lang`, `module:matching`, `module:member-api`, `module:member`, `module:moderation`, `module:notification`, `module:profile`, `module:wave`

---

## 0. 경계 — 이 디렉터리 밖으로 나가지 않는다

**이 모듈 디렉터리(`app/api`) 밖의 파일은 읽지도 고치지도 않는다.** 저장소 루트 `CLAUDE.md`, `module/CLAUDE.md`, 다른 모듈의 소스, `README.md` 전부 열지 않는다. 필요한 규약은 전부 이 문서에 옮겨 담았다 — **이 파일이 이 모듈의 유일한 규약 문서다.**
...
조립과 실행. 스프링 부트 진입점, 전체 설정, 여러 모듈을 가로지르는 E2E 테스트.

- **설정은 `src/main/resources/` 두 파일에만 둔다.** `application.yml`(기본값이자 로컬용, `docker/` 인프라 설정과 짝을 맞춘다. 여기 든 로컬 dummy 시크릿은 운영에서 안 쓰이므로 커밋해도 된다)과 `application-production.yml`(운영용, 민감값은 전부 `${ENV_VAR}` 주입).
- **새 모듈은 `build.gradle.kts` 에 `implementation(project(":module:<name>"))` 를 직접 추가해야 앱에 실린다.** `settings.gradle.kts` 가 자동 등록하는 건 서브프로젝트일 뿐이고, 빠뜨려도 컴파일과 모듈 단위 테스트는 통과하므로 조용히 넘어간다.
- **계약 모듈(`module/*-api`)은 여기 등록하지 않는다.** 빈이 없는 인터페이스·DTO 뿐이라 소비 모듈이 `implementation` 으로 물면 런타임 클래스패스에 그대로 올라온다.
- `logback-spring.xml` 은 `common` 과 여기 양쪽에 의도적으로 둔다. 중복으로 보고 지우지 않는다.
- 로그 파일 경로는 `APP_LOG_PATH` 기본값이 `build/test-logs` 라 테스트가 소스 옆을 더럽히지 않고, `bootRun` 만 `app/log/langlez-server/logs` 를 주입해 Promtail 이 수집한다.
- **보안에 직결되는 값에 `@Value("${key:fallback}")` 같은 조용한 기본값을 넣지 않는다.** 운영에서 프로퍼티가 빠져도 부팅해 문제를 숨긴다.

## 8. 회귀 방지 테스트

- `MessageSourceAutoConfigurationTest` — 로케일 접미사 없는 `messages.properties` 가 사라지면 전 응답이 i18n 키 문자열로 나가는 사고를 막는다.
- **여러 모듈을 가로지르는 E2E 는 여기 둔다.** 모듈 하나짜리 컨텍스트로는 조립이 안 된다. `@SpringBootTest(webEnvironment = RANDOM_PORT)` + `TestRestTemplate`.
- **어노테이션 누락은 리플렉션으로 고정한다.** `@Scheduled`·`@DistributedLock` 이 빠져도 컴파일과 테스트가 통과한다.

## 9. 실행

```bash
./gradlew :app:api:bootRun     # 인프라 기동이 선행돼야 한다 (이미 루트에서 떠 있다)
```
